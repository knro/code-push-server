...
export class AzureStorage implements storage.Storage
{
  ...
  public getBlobUrl(blobId: string): q.Promise<string>
  {
    return this._setupPromise
      .then(() =>
      {
        if (process.env.EMULATED && process.env.SERVER_URL)
        {
          const cleanBlobId = blobId.includes('http://') ? blobId.split('/').pop() : blobId;
          return `${process.env.SERVER_URL}/blob/${AzureStorage.TABLE_NAME}/${cleanBlobId}`;
        }
  ...
  public proxyBlob(container: string, blobId: string, res: any): q.Promise<void>
  {
    return q.Promise<void>((resolve, reject) => {
      this._blobService
